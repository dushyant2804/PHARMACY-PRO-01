...
BUILD_RE = re.compile(r"[0-9]{8}-[A-Za-z0-9][A-Za-z0-9._-]*")
...
def validate_release(key: str, release: ReleaseMetadata) -> None:
    ...
    if not BUILD_RE.fullmatch(release["build_id"]):
        raise ValueError(f"Build ID must be a date-prefixed build identifier: {release['build_id']}")
...
def is_update_available(current: ReleaseMetadata, latest: ReleaseMetadata) -> bool:
    ...
    if semantic_compare < 0:
        return False
    return latest["release_notes"] != current["release_notes"]


def get_version_metadata(current_version: str | None = None, current_build: str | None = None) -> VersionMetadata:
    ...
    Build-only changes with identical semantic version and notes are not marked
    as updates.
    """
    ...
    latest = RELEASES[LATEST_RELEASE_KEY]
